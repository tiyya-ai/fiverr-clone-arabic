...
export async function seedDatabase() {
  ...
  const admin = await prisma.user.upsert({
    ...
    create: {
      ...
      skills: JSON.stringify(['إدارة', 'تطوير الأعمال']),
  ...
  const providers = [
    {
      ...
      skills: JSON.stringify(['كهرباء منزلية', 'كهرباء تجارية', 'إضاءة LED', 'لوحات كهربائية']),
    ...
    },
    {
      ...
      skills: JSON.stringify(['تنظيف منازل', 'تنظيف مكاتب', 'تنظيف بعد الدهان', 'تنظيف السجاد']),
      languages: JSON.stringify(['العربية', 'الإنجليزية']),
      rating: 4.9,
      totalReviews: 203,
      totalSales: 445,
